...
def _make_job(job_id: str, brief: str = "test brief", performance=None, page_name: str = "NayzFreedom Fleet") -> ContentJob:
    job = ContentJob(
        id=job_id,
        project="slay_hack",
...
def test_reporter_aggregates_jobs_in_window(tmp_path):
    ...
    assert "NayzFreedom Fleet" in data
    assert data["NayzFreedom Fleet"]["facebook"].job_count == 1
    assert data["NayzFreedom Fleet"]["facebook"].total_reach == 1000
...
def test_reporter_aggregates_metrics_by_platform(tmp_path):
    ...
    stats = data["NayzFreedom Fleet"]["facebook"]
...
def test_reporter_skips_corrupt_job_file(tmp_path):
    page_dir = tmp_path / "output" / "NayzFreedom Fleet" / IN_WINDOW_ID
...
def test_reporter_top_job_tiebreaker_earlier_id_wins(tmp_path):
    ...
    assert data["NayzFreedom Fleet"]["facebook"].top_job_id == "20260511_060000"
...
def test_reporter_aggregates_multi_platform_job(tmp_path):
    ...
    assert data["NayzFreedom Fleet"]["facebook"].total_reach == 1000
    assert data["NayzFreedom Fleet"]["instagram"].total_reach == 2000
...
def test_reporter_none_recorded_at_snapshot_not_preferred_over_dated(tmp_path):
    ...
    assert data["NayzFreedom Fleet"]["facebook"].total_reach == 800
...
def test_reporter_writes_markdown_file(tmp_path):
    ...
    report_path = tmp_path / "output" / "NayzFreedom Fleet" / f"weekly_report_{date.today()}.md"
